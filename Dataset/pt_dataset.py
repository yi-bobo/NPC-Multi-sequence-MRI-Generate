...
# # region 数据集测试
# import sys
# sys.path.append('/data1/weiyibo/NPC-MRI/Code/NPC-Multi-sequence-MRI-Generate/Dataset')

# split_dir = './Split/zhongshan2'
# mode = 'train'  # 模式：train|val|test
# with open(os.path.join(split_dir, f'{mode}_with_info.txt'), 'r') as f:
#     data_list = []
#     for line in f.readlines():
#         data_list.append(line)

# data = torch.load(os.path.join(split_dir, mode + '_with_info.pt'))
# print(len(data))
# print(data[0])
# # endregion
...
class npy_3D_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        T1_tumor_data = T1_data * T1C_mask_data
        ...
        T1_tumor_data = self.norm_method(T1_tumor_data, max_value, min_value)
        ...
        T1_tumor_data = resize_arr(T1_tumor_data, self.img_size)
        ...
        T1_mask_data = torch.from_numpy(T1C_mask_data).float().unsqueeze(0)
        T1_tumor_data = torch.from_numpy(T1_tumor_data).float().unsqueeze(0)
        ...
        return T1_data, T1_mask_data, T1_tumor_data,  T1C_data, T1C_mask_data, T1C_tumor_data, T2_data, T2_mask_data, T2_tumor_data, patient_id, txt
