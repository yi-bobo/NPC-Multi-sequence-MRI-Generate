...
def test_model(model, val_loader, epoch, opt, best_metrics, model_components):
    """验证模型"""
...
def main():
    # 1️⃣ 加载配置
    config_path = "./Config/Comparative_experiment/Pix2Pix/T1_T1C.yaml"
    ...
    test_model_components = {'net_G': model.net_G, 'net_D': model.net_D, 'optimizer_G': model.optimizer_G, 'optimizer_D': model.optimizer_D}
    epoch = load_model(test_model_components, checkpoint_path=opt.path.checkpoint_path, device=opt.train.device)
    ...
    best_metrics = test_model(model, val_loader, epoch, opt, best_metrics, test_model_components)
