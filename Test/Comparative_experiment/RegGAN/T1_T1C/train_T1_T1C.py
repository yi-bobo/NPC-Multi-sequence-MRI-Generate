...
from Model.RegGAN_model import RegGANModel
...
def main():
    # 1️⃣ 加载配置
    config_path = "./Config/Comparative_experiment/RegGAN/T1_T1C.yaml"
    ...
    # 5️⃣ 创建模型
    model = RegGANModel(opt).to(opt.train.device)
    model_components = {'netG': model.netG.state_dict(), 'netG': model.netG.state_dict(), 'netR': model.netR.state_dict(), 
                        'optimizer_G': model.optimizer_G.state_dict(), 'optimizer_D': model.optimizer_D.state_dict(), 'optimizer_R': model.optimizer_R.state_dict()}
    if opt.train.continue_train:
        logging.info("加载已保存模型参数")
        val_model = {'netG': model.netG, 'netG': model.netG, 'netR': model.netR, 
                        'optimizer_G': model.optimizer_G, 'optimizer_D': model.optimizer_D, 'optimizer_R': model.optimizer_R}
