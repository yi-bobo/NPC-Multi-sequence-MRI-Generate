...
def validate_model(model, val_loader, epoch, opt, best_metrics):
    ...
    with torch.no_grad():
        ...
        # for i, data in zip(range(2), val_loader):
            ...
            # 保存单个样本验证结果
            test_cal_dict = {'patient_id': patient_id, 'MAE_targ': mae_targ, 'SSIM_targ': SSIM_targ, 'PSNR_targ': PSNR_targ, 
            ...
            model.plot(input, target, pred_target, pred_input, data_mapping, save_path_dir)
    ...
    logging.info(f"测试集epoch_{epoch}模型的验证结果: {metrics_str}")
