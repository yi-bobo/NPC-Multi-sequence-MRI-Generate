...
from Model.Net.CPC_BM_Net.UNet import ConditionalDiffusionModelUNet
...
class LossManager(nn.Module):
    def __init__(self, loss_type, lambda_con, lambda_rec, lambda_cycle, lambda_perceptual, use_perceptual=False, device="cuda", multi_gpu=False):
        super().__init__()
        ...
        self.perceptual_loss = VGGLoss_3D(device=device, multi_gpu=multi_gpu) if use_perceptual else None

    def compute(self, preds, targets, mode='f'):
        # preds = dict of pred_x0_xt, pred_x0_xT, pred_cycle
        # //?一致性损失
        loss_con = self.loss_fn(preds['rec_t'], preds['rec_T'])
        # //? 重建损失
        loss_rec = 0.5 * (self.loss_fn(preds['rec_t'], targets['gt']) + self.loss_fn(preds['rec_T'], targets['gt']))
        # //? 循环一致性损失
        loss_cycle = self.loss_fn(preds['cycle'], targets['gt'])

        loss = self.lambda_con * loss_con + self.lambda_rec * loss_rec + self.lambda_cycle * loss_cycle
        loss_dict = {'con': loss_con.item(), 'rec': loss_rec.item(), 'cycle': loss_cycle.item()}

        if self.use_perceptual:
            loss_perc = (
                self.perceptual_loss(preds['rec_t'], targets['gt']) +
                self.perceptual_loss(preds['rec_T'], targets['gt']) +
                self.perceptual_loss(preds['cycle'], targets['gt'])
            ) / 3.0
            loss += self.lambda_perceptual * loss_perc
...
class CPC_BM(nn.Module):
    def __init__(self, opt):
        ...
        self.loss_manager = LossManager(
            loss_type=opt.loss.loss_type,
            lambda_con=opt.loss.lambda_con,
            lambda_rec=opt.loss.lambda_rec,
            lambda_cycle=opt.loss.lambda_cycle,
            lambda_perceptual=opt.loss.lambda_perceptual,
            use_perceptual=opt.loss.is_perceptual,
            device=self.device,
            multi_gpu=self.multi_gpu
        )

        # Optimizer + Scheduler
        params = itertools.chain(self.net_f.parameters(), self.net_b.parameters())
        self.optimizer = create_optimizer(opt.optim.optimizer.name, params, **vars(opt.optim.optimizer.params))
        self.scaler = GradScaler()  # mixed precision
        self.scheduler = GradualWarmupScheduler(
            self.optimizer,
            multiplier=opt.optim.scheduler.params.multiplier,
            warm_epoch=100,
            after_scheduler=torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=opt.train.max_epochs)
        )
    ...
    def forward(self, x_0, x_T, text_con=None, img_con1=None, img_con2=None, global_step=0):
        ...
        with autocast():
            ...
            # F direction 
            with torch.no_grad():
                pred_xT_from_x0 = self.net_b(x_0.detach(), timesteps=T, text_features=text_con, cond_image=img_con1)
            preds_f = {
                'rec_t': self.net_f(x_t, timesteps=t, text_features=text_con, cond_image=img_con2),
                'rec_T': self.net_f(x_T, timesteps=T, text_features=text_con, cond_image=img_con2),
                'cycle': self.net_f(pred_xT_from_x0, timesteps=T, text_features=text_con, cond_image=img_con2)
            }
            loss_f, loss_f_dict = self.loss_manager.compute(preds_f, {'gt': x_0}, mode='f')

            # B direction
            with torch.no_grad():
                pred_x0_from_xT = self.net_f(x_T.detach(), timesteps=T, text_features=text_con, cond_image=img_con2)
            preds_b = {
                'rec_t': self.net_b(x_t, timesteps=(T - t), text_features=text_con, cond_image=img_con1),
                'rec_T': self.net_b(x_0, timesteps=T, text_features=text_con, cond_image=img_con1),
                'cycle': self.net_b(pred_x0_from_xT, timesteps=T, text_features=text_con, cond_image=img_con1)
            }
            loss_b, loss_b_dict = self.loss_manager.compute(preds_b, {'gt': x_T}, mode='b')

            loss = loss_f + loss_b

            loss_dict = {
                'loss': loss.item(),
                **loss_f_dict, **loss_b_dict
            }

        self.scaler.scale(loss/self.accumulate_grad_batches).backward()  # 使用混合精度进行反向传播

        if (global_step + 1) % self.accumulate_grad_batches == 0:
        ...
        time2 = time.time()
        print(f"Forward time: {time2 - time1:.4f}s")
        
        return loss, {**loss_f_dict, **loss_b_dict}
