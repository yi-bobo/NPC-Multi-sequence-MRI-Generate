...
class CPC_BM(nn.Module):
    ...
    def backward_f(self, x_0, x_t, x_T, t, T, text_features=None, img_features=None):
        "x_T->x_0 "
        pred_x0_from_xt = self.net_f(x_t, timesteps=t)
        pred_x0_fron_xT = self.net_f(x_T, timesteps=T)
        with torch.no_grad():
            pred_xT_from_x0 = self.net_b(x_0.detach(), timesteps=t)
        pred_x0_from_x0 = self.net_f(pred_xT_from_x0.detach(), timesteps=T)
    ...
    def backward_b(self, x_0, x_t, x_T, t, T, text_features=None, img_features=None):
        " x_0->x_T "
        pred_xT_from_xt = self.net_b(x_t, timesteps=(T-t))
        pred_xT_from_x0 = self.net_b(x_0, timesteps=T)
        with torch.no_grad():
            pred_x0_from_xT = self.net_f(x_T.detach(), timesteps=T)
        pred_xT_from_xT = self.net_b(pred_x0_from_xT.detach(), timesteps=T)
    ...
    def p_losses(self, x_0, x_T, t, T, text_con=None, img_con1=None, img_con2=None, noise=None):
        ...
        loss_f, loss_f_dict = self.backward_f(x_0, x_t, x_T, t, T, text_con, img_con2)
        end_time = time.time()  # 记录结束时间
        print(f"Backward pass for net_f took {end_time - start_time:.2f} seconds")
        torch.cuda.empty_cache()  # 清理 CUDA 缓存，释放内存

        # 2️⃣ 计算 net_b 的损失
        # x_0 -> x_T
        start_time = time.time()  # 记录开始时间
        loss_b, loss_b_dict = self.backward_b(x_0, x_t, x_T, t, T, text_con, img_con1)
        end_time = time.time()  # 记录结束时间
        print(f"Backward pass for net_b took {end_time - start_time:.2f} seconds")
    ...
    def forward(self, x_0, x_T, 
        ...
        print(f"迭代一次需要花费 {time2 - time1:.2f} s")
